...
SED_VALIDAR_URL = "https://sedintegracoes.educacao.sp.gov.br/credenciais/api/ValidarToken"
SED_KEY = "2b03c1db3884488795f79c37c069381a"
...
# -------------------- HELPERS --------------------
def validar_token_sed(auth_token: str) -> bool:
    """Valida o token do usuário na SED"""
    try:
        headers = {
            "authorization": auth_token,
            "ocp-apim-subscription-key": SED_KEY,
            "accept": "application/json, text/plain, */*",
        }
        r = requests.post(SED_VALIDAR_URL, headers=headers, timeout=10)
        logging.info("ValidarToken SED -> %s", r.status_code)
        return r.status_code == 200
    except Exception as e:
        logging.error("Erro ao validar token SED: %s", e)
        return False
...
def process_one_task(token, task_obj, time_min=1, time_max=3, is_draft=False):
    try:
        ...
        if not validar_token_sed(token):
            return {"success": False, "message": "Token inválido na SED", "task_id": task_obj.get("id")}
...
def auth():
    try:
        ...
        j = r.json()
        token = j.get("auth_token")

        # 🔑 valida token na SED
        if not validar_token_sed(token):
            return jsonify({"success": False, "message": "Token inválido na SED"}), 401

        logging.info("DEBUG /auth login OK: ra=%s nick=%s", ra, j.get("nick"))
        return jsonify({"success": True, "auth_token": token, "nick": j.get("nick")})
    except Exception as e:
...
def tasks():
    ...
    try:
        ...
        base_params = {"limit": 100, "offset": 0, "with_answer": "true"}
